...
class CapitalAllocator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.allocation_strategy = AllocationStrategy(
            self.config.get("strategy", "equal_weight")
        )
    ...
    def rebalance_portfolio(
        ...
        try:
            ...
            all_protocols = set(current_allocations.keys()) | set(target_allocations.keys())
            ...
            for protocol in all_protocols:
                ...
                if abs(deviation) >= self.rebalance_threshold:
                    ...
                    priority = int(abs(deviation) * 100)  # Higher deviation = higher priority
    ...
    def calculate_efficiency_metrics(
        ...
        try:
            ...
            std_return = Decimal(str(np.std(returns_array, ddof=1)))  # Use sample std dev
            ...
            protocol_allocations = {"scroll": Decimal("0.30"), "zksync": Decimal("0.70")}
    ...
    def _risk_parity_allocation(
        ...
        inverse_risks = {p: Decimal("1") / risk_scores[p] for p in protocols}
        total_inverse_risk = sum(inverse_risks.values())

        # Normalize to get allocations
        allocations = {}
        max_exposure = risk_constraints.get("max_protocol_exposure_pct", Decimal("100")) / Decimal("100")

        for protocol in protocols:
            allocation = inverse_risks[protocol] / total_inverse_risk
    ...
    def _mean_variance_allocation(
        ...
        for protocol in protocols:
            risk_adjusted_return = expected_returns[protocol] / risk_scores[protocol]
        ...
        for protocol in protocols:
            allocation = risk_adjusted_returns[protocol] / total_risk_adjusted
            allocations[protocol] = min(allocation, max_exposure)
