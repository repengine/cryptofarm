...
class TestRiskManager:
    ...
    def test_monitor_positions_multiple_wallets(self, risk_manager):
        ...
        mock_web3 = Mock()
        mock_web3.is_connected.return_value = True
        mock_web3.eth.get_balance.return_value = 500000000000000000  # 0.5 ETH in wei
        ...
        with patch.object(risk_manager, '_get_eth_price_usd', return_value=Decimal('2000')):
            ...
            assert 'ethereum' in exposures
            assert exposures['ethereum'] == Decimal('2000')  # 2 wallets * 0.5 ETH * $2000
    ...
    def test_monitor_positions_multiple_networks(self, risk_manager):
        ...
        mock_scroll_web3.eth.get_balance.return_value = 500000000000000000  # 0.5 ETH
        ...
        with patch.object(risk_manager, '_get_eth_price_usd', return_value=Decimal('2000')):
            ...
            assert exposures['ethereum'] == Decimal('2000')  # 1 ETH * $2000
            assert exposures['scroll'] == Decimal('1000')    # 0.5 ETH * $2000
    ...
    def test_monitor_positions_disconnected_provider(self, risk_manager):
        ...
        with patch.object(risk_manager, '_get_eth_price_usd', return_value=Decimal('2000')):
            ...
            # Should skip disconnected providers
            assert exposures == {}
    ...
    def test_monitor_gas_costs_high_price_warning(self, risk_manager):
        ...
        mock_web3 = Mock()
        mock_web3.is_connected.return_value = True
        mock_web3.eth.gas_price = 150000000000  # 150 Gwei in wei (above limit)
    ...
    def test_monitor_market_volatility_extreme(self, risk_manager):
        ...
        # Mock extreme volatility scenario
        with patch.object(risk_manager, 'monitor_market_volatility') as mock_method:
    ...
    def test_monitor_market_volatility_multiple_assets(self, risk_manager):
        ...
        # The current implementation uses mock data, so this tests the logic
        volatility = risk_manager.monitor_market_volatility(['ETH', 'BTC', 'USDC'])
        assert volatility in [VolatilityState.LOW, VolatilityState.MEDIUM, VolatilityState.HIGH, VolatilityState.EXTREME]
    ...
    def test_get_eth_price_usd_fallback(self, risk_manager):
        ...
        # Test the actual fallback behavior in the method
        with patch('airdrops.risk_management.core.logger'):
            # The method already has fallback logic built-in
            price = risk_manager._get_eth_price_usd()
            assert price == Decimal('2000.0')  # Mock price
...
class TestRiskManagerAdvancedMethods:
    ...
    def test_assess_current_risk_success(self, risk_manager):
        ...
        mock_exposures = {'ethereum': Decimal('5000'), 'scroll': Decimal('3000')}
        
        with patch.object(risk_manager, 'monitor_positions', return_value=mock_exposures), \
             patch.object(risk_manager, 'monitor_gas_costs', return_value=Decimal('50')), \
            ...
            assert metrics.portfolio_value == Decimal('8000')  # 5000 + 3000
            assert metrics.portfolio_pnl == Decimal('400')     # 8000 * 0.05
    ...
    def test_assess_current_risk_high_risk_scenario(self, risk_manager):
        ...
        with patch.object(risk_manager, 'monitor_positions', return_value=mock_exposures), \
             patch.object(risk_manager, 'monitor_gas_costs', return_value=Decimal('200')), \
    ...
    def test_assess_current_risk_circuit_breaker_triggered(self, risk_manager):
        ...
        with patch.object(risk_manager, 'monitor_positions', return_value=mock_exposures), \
             patch.object(risk_manager, 'monitor_gas_costs', return_value=Decimal('300')), \
             patch.object(risk_manager, 'monitor_market_volatility', return_value=VolatilityState.EXTREME), \
             patch.object(risk_manager, 'trigger_circuit_breaker') as mock_trigger:
            
            metrics = risk_manager.assess_current_risk(['0x123'])
            
            # Should trigger circuit breaker for extreme conditions
    ...
    def test_assess_current_risk_failure(self, risk_manager):
        ...
        with patch.object(risk_manager, 'monitor_positions', side_effect=Exception("Monitor error")):
    ...
    def test_trigger_circuit_breaker_failure(self, risk_manager):
        ...
        with patch('airdrops.risk_management.core.logger') as mock_logger:
            ...
            with pytest.raises(RuntimeError, match="Circuit breaker activation failed"):
                risk_manager.trigger_circuit_breaker()
    ...
    def test_calculate_position_size_limits_success(self, risk_manager):
        ...
        with patch.object(risk_manager, 'monitor_market_volatility', return_value=VolatilityState.MEDIUM):
            ...
            assert limits['volatility_adjustment'] == Decimal('0.8')  # Medium volatility
    
    def test_calculate_position_size_limits_high_volatility(self, risk_manager):
        ...
        with patch.object(risk_manager, 'monitor_market_volatility', return_value=VolatilityState.HIGH):
            ...
            assert limits['volatility_adjustment'] == Decimal('0.6')  # High volatility
            # Position sizes should be reduced due to high volatility
            assert limits['max_position_size'] < Decimal('2000')  # 20% * 0.6 = 12%
    
    def test_calculate_position_size_limits_extreme_volatility(self, risk_manager):
        ...
        with patch.object(risk_manager, 'monitor_market_volatility', return_value=VolatilityState.EXTREME):
            ...
            assert limits['volatility_adjustment'] == Decimal('0.3')  # Extreme volatility
            # Position sizes should be significantly reduced
            assert limits['max_position_size'] < Decimal('1000')  # 20% * 0.3 = 6%
    ...
    def test_check_emergency_stop_conditions_critical_risk(self, risk_manager):
    ...
    def test_check_emergency_stop_conditions_excessive_loss(self, risk_manager):
    ...
    def test_check_emergency_stop_conditions_extreme_volatility(self, risk_manager):
    ...
    def test_check_emergency_stop_conditions_protocol_concentration(self, risk_manager):
        ...
        metrics = RiskMetrics(
            ...
            gas_price_gwei=Decimal('50'),
            volatility_state=VolatilityState.MEDIUM,
            protocol_exposures={'ethereum': Decimal('3500')},  # 35% > 30% limit
            risk_level=RiskLevel.MEDIUM,
            circuit_breaker_triggered=False
        )
    ...
    def test_check_emergency_stop_conditions_normal(self, risk_manager):
        ...
        metrics = RiskMetrics(
            ...
            gas_price_gwei=Decimal('50'),
            volatility_state=VolatilityState.MEDIUM,
            protocol_exposures={'ethereum': Decimal('1500'), 'scroll': Decimal('1000')},
    ...
    def test_check_emergency_stop_conditions_failure(self, risk_manager):
        ...
        metrics = RiskMetrics(
            portfolio_value=Decimal('0'),  # Will cause division by zero
            portfolio_pnl=Decimal('500'),
            gas_price_gwei=Decimal('50'),
            volatility_state=VolatilityState.MEDIUM,
        ...
        # Should return True (err on side of caution) when error occurs
        result = risk_manager.check_emergency_stop_conditions(metrics)
        assert result is True
    ...
    def test_calculate_risk_level_low(self, risk_manager):
        ...
        risk_level = risk_manager._calculate_risk_level(
            portfolio_value=Decimal('10000'),
            gas_price=Decimal('30'),  # Below limit
            volatility_state=VolatilityState.LOW,
            protocol_exposures={'ethereum': Decimal('1000')}  # 10% exposure
        )
    ...
    def test_calculate_risk_level_medium(self, risk_manager):
        ...
        risk_level = risk_manager._calculate_risk_level(
            portfolio_value=Decimal('10000'),
            gas_price=Decimal('85'),  # Above 80% of limit
            volatility_state=VolatilityState.MEDIUM,
            protocol_exposures={'ethereum': Decimal('1500')}  # 15% exposure
        )
    ...
    def test_calculate_risk_level_high(self, risk_manager):
        ...
        risk_level = risk_manager._calculate_risk_level(
            portfolio_value=Decimal('10000'),
            gas_price=Decimal('85'),  # Below limit but above 80% threshold
            volatility_state=VolatilityState.HIGH,  # +2 points
            protocol_exposures={'ethereum': Decimal('1500')}  # 15% exposure (below limit)
        )
        
        assert risk_level == RiskLevel.HIGH  # Should be 3 points total
    
    def test_calculate_risk_level_critical(self, risk_manager):
        """Test risk level calculation - critical risk."""
        risk_level = risk_manager._calculate_risk_level(
            portfolio_value=Decimal('10000'),
            gas_price=Decimal('150'),  # Well above limit
            volatility_state=VolatilityState.EXTREME,
            protocol_exposures={'ethereum': Decimal('3000')}  # 30% exposure
        )
        
        assert risk_level == RiskLevel.CRITICAL
    
    def test_should_trigger_circuit_breaker_critical_risk(self, risk_manager):
    ...
    def test_should_trigger_circuit_breaker_extreme_gas(self, risk_manager):
        ...
        result = risk_manager._should_trigger_circuit_breaker(
            risk_level=RiskLevel.MEDIUM,
            portfolio_pnl=Decimal('100'),
            gas_price=Decimal('250')  # 2.5x the limit
        )
        
        assert result is True
    
    def test_should_trigger_circuit_breaker_already_active(self, risk_manager):
    ...
    def test_get_volatility_multiplier_all_states(self, risk_manager):
        ...
        assert risk_manager._get_volatility_multiplier(VolatilityState.LOW) == Decimal('1.0')
        assert risk_manager._get_volatility_multiplier(VolatilityState.MEDIUM) == Decimal('0.8')
        assert risk_manager._get_volatility_multiplier(VolatilityState.HIGH) == Decimal('0.6')
        assert risk_manager._get_volatility_multiplier(VolatilityState.EXTREME) == Decimal('0.3')
...
class TestRiskMetrics:
    ...
    def test_risk_metrics_with_negative_pnl(self):
        ...
        metrics = RiskMetrics(
            ...
            protocol_exposures={'ethereum': Decimal('4000'), 'scroll': Decimal('4000')},
...
class TestRiskManagerEdgeCases:
    ...
    def test_monitor_gas_costs_network_error(self, risk_manager):
        ...
        mock_web3 = Mock()
        mock_web3.is_connected.return_value = True
        mock_web3.eth.gas_price = property(lambda self: (_ for _ in ()).throw(ConnectionError("Network error")))
