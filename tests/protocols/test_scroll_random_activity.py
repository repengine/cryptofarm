"""
Unit tests for the perform_random_activity function in the Scroll protocol.
...
from airdrops.protocols.scroll.scroll import perform_random_activity
from airdrops.protocols.scroll.exceptions import ScrollRandomActivityError
...
class TestPerformRandomActivity:
    ...
    def test_successful_execution_first_attempt(
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
             patch('airdrops.protocols.scroll.scroll._execute_scroll_activity') as mock_execute:
            ...
            mock_execute.assert_called_once_with(
                ...
                "0x" + "a" * 64,
                valid_config["random_activity"]["scroll"]["amount_ranges"],
                valid_config["random_activity"]["scroll"]["token_config"],
                mock_web3_l1,
                mock_web3_l2
            )
    ...
    def test_fallback_logic_second_attempt_success(
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
            ...
            mock_select.side_effect = ["swap", "lend"]
            ...
            assert results[1]["activity"] == "lend"
    ...
    def test_max_retries_failure(
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
            ...
            mock_select.side_effect = ["swap", "lend", "bridge"]
            mock_execute.side_effect = [
                Exception("Swap failed"),
                Exception("Lend failed"),
                Exception("Bridge failed")
            ]
            
            # Execute function and expect exception
            with pytest.raises(ScrollRandomActivityError, match="All random activities failed after 3 attempts"):
    ...
    def test_parameter_generation_swap_activity(
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
            ...
            # Verify _execute_scroll_activity was called with correct parameters
            mock_execute.assert_called_once_with(
                "swap",
                "0x742d35Cc6634C0532925a3b844Bc9e7195Ed5E47",
                "0x" + "a" * 64,
    ...
    def test_parameter_generation_lend_activity(
        self, 
        valid_config: Dict[str, Any], 
        mock_web3_instances: tuple[MagicMock, MagicMock]
    ) -> None:
        """Test that correct parameters are generated for lend activity."""
        mock_web3_l1, mock_web3_l2 = mock_web3_instances
    ...
    def test_invalid_configuration_missing_scroll_section(
        self, 
        mock_web3_instances: tuple[MagicMock, MagicMock]
    ) -> None:
        """Test handling of missing scroll section in configuration."""
        mock_web3_l1, mock_web3_l2 = mock_web3_instances
        ...
        with pytest.raises(ValueError, match="config must contain 'random_activity.scroll' section"):
            perform_random_activity(
                user_address="0x742d35Cc6634C0532925a3b844Bc9e7195Ed5E47",
                private_key="0x" + "a" * 64,
                config=invalid_config,
    ...
    def test_activity_pool_removal_on_failure(
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
            ...
            assert any(activity["name"] == "lend" for activity in second_call_args)
            assert any(activity["name"] == "bridge" for activity in second_call_args)

    def test_provide_liquidity_activity_path(
        self, 
        mock_web3_instances: tuple[MagicMock, MagicMock]
    ) -> None:
        """Test that provide_liquidity activity is handled correctly."""
        mock_web3_l1, mock_web3_l2 = mock_web3_instances
        
        config_with_liquidity = {
            "random_activity": {
                "scroll": {
                    "action_weights": [
                        {"name": "provide_liquidity", "weight": 100}
                    ],
                    "max_retries": 3,
                    "amount_ranges": {
                        "provide_liquidity": {"min": "0.01", "max": "0.1", "decimals": 4}
                    },
                    "token_config": {
                        "ETH": {"address": "0x123"},
                        "USDC": {"address": "0x456"}
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
            ...
            # Setup mocks
            mock_select.return_value = "provide_liquidity"
            mock_execute.return_value = "0xabcdef"
            ...
            # Verify results
            assert len(results) == 1
            assert results[0]["activity"] == "provide_liquidity"
            assert results[0]["status"] == "success"
            ...
            mock_execute.assert_called_once_with(
                ...
                "0x742d35Cc6634C0532925a3b844Bc9e7195Ed5E47",
                "0x" + "a" * 64,
                config_with_liquidity["random_activity"]["scroll"]["amount_ranges"],
                config_with_liquidity["random_activity"]["scroll"]["token_config"],
                mock_web3_l1,
                mock_web3_l2
            )
    ...
    def test_config_defaults_handling(
        ...
        with patch('airdrops.protocols.scroll.scroll.select_activity_by_weight') as mock_select, \
            ...
            # Verify _execute_scroll_activity was called with empty defaults
            mock_execute.assert_called_once_with(
                "swap",
                "0x742d35Cc6634C0532925a3b844Bc9e7195Ed5E47",
                "0x" + "a" * 64,
